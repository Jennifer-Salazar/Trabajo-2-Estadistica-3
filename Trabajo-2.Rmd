...
mod_local_ajust <- mod3_Tt + St # Ajuste D&LC(AICC)
```


```{r}
plot(Datos20)
lines(mod_local_ajust, col=2, lwd=2)
...
plot(et_local,ylim=c(min(-2*sqrt(MSE_local),et_local),max(2*sqrt(MSE_local),et_local)))
abline(h=c(-2*sqrt(MSE_local),0,2*sqrt(MSE_local)),col=2)
legend("topleft", legend=c("Modelo local"), lty=1, col = 1, lwd=2)
...
```{r}
plot(as.numeric(mod_local_ajust),et_local,ylim=c(min(-2*sqrt(MSE_local),et_local),max(2*sqrt(MSE_local),et_local)))
abline(h=c(-2*sqrt(MSE_local),0,2*sqrt(MSE_local)),col=2)
legend("topleft", legend=c("Modelo local"), lty=1, col = 1, lwd=2,cex = 0.8)
```
...
qqnorm(et_local,main="Gráfico de normalidad residuos de ajuste modelo1")
