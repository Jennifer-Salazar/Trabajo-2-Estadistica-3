...
```{r, fig.height=5, fig.width=6}
par(bg='gray98')
acf(Datos20,lag.max=36,ci.type="ma",col="cyan4", ci.col=2, lwd=2)
title(main = 'Hola')
grid()
```
...
legend("topleft", legend = c("Original", "Ajuste"), lty=1, col=c(1,2))
grid()
```
...
```{r, fig.height=5, fig.width=6}
acf(residuales,lag.max=36,ci.type="ma",col="cyan4",ci.col=2, lwd=2)
```

```{r, fig.height=5, fig.width=6}
pacf(residuales,lag.max=36,col="cyan4",ci.col=2, lwd=2)
```
